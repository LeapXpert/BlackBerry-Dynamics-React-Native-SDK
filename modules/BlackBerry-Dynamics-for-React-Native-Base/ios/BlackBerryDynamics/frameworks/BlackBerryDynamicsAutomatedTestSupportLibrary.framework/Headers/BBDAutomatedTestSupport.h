 ...
 * In case current activation type is "idp", then "Sign in" option is selected,
 * Otherwise, the default provisioning flow will be triggered by selecting "Enter credentials" option.
 *
 *  @return YES, if activation type was selected, otherwise NO
 */
- (BOOL)selectActivationType;
...
/**
 * Method that inputs email for IDP activation and submits it (by clicking Go button)
 */
- (BOOL)processEmailIDP:(NSString *)email;

/**
 * Method that inputs username for IDP activation and submits it (by clicking Go button)
 */
- (BOOL)processUsernameIDP:(NSString *)username;

/**
 * Method that inputs password for IDP activation and submits it (by clicking Go button)
 */
- (BOOL)processPasswordIDP:(NSString *)password;
